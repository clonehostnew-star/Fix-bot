...
global.channelLink = "https://whatsapp.com/channel/0029Val3Ewv6xCSGCE9fZD0H";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // WhatsApp Master: protection pass
        try {
            const protect = await handleIncomingProtection(sock, message);
            if (protect?.blocked) return;
        } catch {}
        ...
        if (userMessage.startsWith('.')) {
            ...
            const cmdKey = userMessage.split(/\s+/)[0].slice(1);
            try { drainForCommand(cmdKey); } catch {}
        }
        ...
        switch (true) {
            ...
            case userMessage === '.help' || userMessage === '.menu' || userMessage === '.bot' || userMessage === '.list':
                ...
                commandExecuted = true;
                break;
            case userMessage.startsWith('.wamaster') || userMessage.startsWith('.wam'):
                {
            ...
            case userMessage.startsWith('.system'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await systemCommand(sock, chatId, message, args);
                }
                break;
            ...
            case userMessage.startsWith('.mode'):
                ...
                if (!action) {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `⚙️ WA BOT TREE — Mode Status\n\n• Current Mode: *${currentMode.toUpperCase()}*\n\nUse one of the following:\n• .mode public  — everyone can use the bot\n• .mode private — only owner/sudo can use the bot`,
                        ...channelInfo
                    }, { quoted: message });
                    return;
                }
                ...
                try {
                    ...
                    await sock.sendMessage(chatId, { text: `✅ Mode updated: *${action.toUpperCase()}*\nTips:\n• public — anyone can use commands\n• private — only owner/sudo`, ...channelInfo });
                } catch (error) {
            ...
            case userMessage.startsWith('.triggered'):
                ...
                break;
            case userMessage.startsWith('.responsory'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await responsoryCommand(sock, chatId, message, args);
                }
                break;
            case userMessage.startsWith('.eco'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await economyCommand(sock, chatId, message, args);
                }
                break;
            case userMessage.startsWith('.hm'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await hangmanGame(sock, chatId, message, args);
                }
                break;
            case userMessage.startsWith('.number'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await numberGame(sock, chatId, message, args);
                }
                break;
            case userMessage.startsWith('.wordhunt'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await wordHunt(sock, chatId, message, args);
                }
                break;
            case userMessage.startsWith('.wordcount'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await wordCount(sock, chatId, message, args);
                }
                break;
            case userMessage.startsWith('.bible') || userMessage.startsWith('.b'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await bibleCommand(sock, chatId, message, args);
                }
