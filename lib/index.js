...
module.exports = {
    ...
    setBibleQuizConfig,
    getBibleQuizConfig,
    // Bible riddle/scramble utilities
    recordBibleRiddleSolo,
    getBibleRiddleLeaderboard,
    resetBibleRiddleLeaderboard,
    setBibleRiddleEnabled,
    isBibleRiddleEnabled,
    setBibleRiddleConfig,
    getBibleRiddleConfig,
    recordBibleScrambleSolo,
    getBibleScrambleLeaderboard,
    resetBibleScrambleLeaderboard,
    setBibleScrambleEnabled,
    isBibleScrambleEnabled,
    setBibleScrambleConfig,
    getBibleScrambleConfig,
    // Bible study helpers
    setBibleStudyVersion,
    getBibleStudyVersion,
}; 
...
function ensureSection(data, key) {
    if (!data[key]) {
        data[key] = { enabled: {}, scores: {}, groupScores: {}, config: {} };
    } else {
        if (!data[key].enabled) data[key].enabled = {};
        if (!data[key].scores) data[key].scores = {};
        if (!data[key].groupScores) data[key].groupScores = {};
        if (!data[key].config) data[key].config = {};
    }
}
...
// ---- Riddle helpers ----
async function recordBibleRiddleSolo(userId, chatId, points, correct, total) {
    return recordGeneric('bibleRiddle', userId, chatId, points, correct, total);
}
async function getBibleRiddleLeaderboard(scope='global', chatId, topN=10) {
    return getGenericLeaderboard('bibleRiddle', scope, chatId, topN);
}
async function resetBibleRiddleLeaderboard(scope='group', chatId) {
    return resetGenericLeaderboard('bibleRiddle', scope, chatId);
}
async function setBibleRiddleEnabled(chatId, enabled) {
    return setGenericEnabled('bibleRiddle', chatId, enabled);
}
async function isBibleRiddleEnabled(chatId) {
    return isGenericEnabled('bibleRiddle', chatId);
}
async function setBibleRiddleConfig(chatId, cfg) {
    return setGenericConfig('bibleRiddle', chatId, cfg);
}
async function getBibleRiddleConfig(chatId) {
    return getGenericConfig('bibleRiddle', chatId);
}

// ---- Scramble helpers ----
async function recordBibleScrambleSolo(userId, chatId, points, correct, total) {
    return recordGeneric('bibleScramble', userId, chatId, points, correct, total);
}
async function getBibleScrambleLeaderboard(scope='global', chatId, topN=10) {
    return getGenericLeaderboard('bibleScramble', scope, chatId, topN);
}
async function resetBibleScrambleLeaderboard(scope='group', chatId) {
    return resetGenericLeaderboard('bibleScramble', scope, chatId);
}
async function setBibleScrambleEnabled(chatId, enabled) {
    return setGenericEnabled('bibleScramble', chatId, enabled);
}
async function isBibleScrambleEnabled(chatId) {
    return isGenericEnabled('bibleScramble', chatId);
}
async function setBibleScrambleConfig(chatId, cfg) {
    return setGenericConfig('bibleScramble', chatId, cfg);
}
async function getBibleScrambleConfig(chatId) {
    return getGenericConfig('bibleScramble', chatId);
}

// ---- Generic helpers for sections ----
async function recordGeneric(section, userId, chatId, points, correct, total) {
    try {
        if (!userId) return false;
        const data = loadUserGroupData();
        ...
        if (!data[section].scores[userId]) {
            data[section].scores[userId] = { points: 0, best: 0, games: 0, correct: 0, total: 0, lastAt: 0 };
        }
        const s = data[section].scores[userId];
        s.points = (s.points || 0) + (points || 0);
        s.games = (s.games || 0) + 1;
        s.correct = (s.correct || 0) + (correct || 0);
        s.total = (s.total || 0) + (total || 0);
        ...
        if (chatId) {
            if (!data[section].groupScores[chatId]) data[section].groupScores[chatId] = {};
            if (!data[section].groupScores[chatId][userId]) {
                data[section].groupScores[chatId][userId] = { points: 0, best: 0, games: 0, correct: 0, total: 0, lastAt: 0 };
            }
            const g = data[section].groupScores[chatId][userId];
            g.points = (g.points || 0) + (points || 0);
            g.games = (g.games || 0) + 1;
            g.correct = (g.correct || 0) + (correct || 0);
            g.total = (g.total || 0) + (total || 0);
        ...
        console.error('Error recording generic result:', e);
        return false;
    }
}

async function getGenericLeaderboard(section, scope='global', chatId, topN=10) {
    try {
        const data = loadUserGroupData();
        ensureSection(data, section);
        const src = scope === 'group' && chatId ? (data[section].groupScores[chatId] || {}) : (data[section].scores || {});
        ...
        console.error('Error getting generic leaderboard:', e);
        return [];
    }
}

async function resetGenericLeaderboard(section, scope='group', chatId) {
    try {
        const data = loadUserGroupData();
        ensureSection(data, section);
        if (scope === 'global') {
            ...
            data[section].groupScores[chatId] = {};
        }
        saveUserGroupData(data);
        return true;
    } catch (e) {
        console.error('Error resetting generic leaderboard:', e);
        return false;
    }
}

async function setGenericEnabled(section, chatId, enabled) {
    try {
        const data = loadUserGroupData();
        ensureSection(data, section);
        data[section].enabled[chatId] = !!enabled;
        saveUserGroupData(data);
        return true;
    } catch (e) {
        console.error('Error setting enabled:', e);
        return false;
    }
}

async function isGenericEnabled(section, chatId) {
    try {
        const data = loadUserGroupData();
        ensureSection(data, section);
        if (!chatId || !chatId.endsWith('@g.us')) return true;
        const val = data[section].enabled[chatId];
        return typeof val === 'boolean' ? val : true;
    } catch (e) {
        console.error('Error reading enabled state:', e);
        return true;
    }
}

async function setGenericConfig(section, chatId, cfg) {
    try {
        const data = loadUserGroupData();
        ensureSection(data, section);
        if (!data[section].config[chatId]) data[section].config[chatId] = {};
        Object.assign(data[section].config[chatId], cfg || {});
        saveUserGroupData(data);
        return data[section].config[chatId];
    } catch (e) {
...
async function getGenericConfig(section, chatId) {
    try {
        const data = loadUserGroupData();
        ensureSection(data, section);
        return data[section].config[chatId] || {};
...
// ---- Bible study helpers ----
async function setBibleStudyVersion(chatId, version) {
    try {
        const data = loadUserGroupData();
        if (!data.bibleStudy) data.bibleStudy = { versionByChat: {} };
        if (!data.bibleStudy.versionByChat) data.bibleStudy.versionByChat = {};
        data.bibleStudy.versionByChat[chatId] = String(version || '').toLowerCase();
        saveUserGroupData(data);
        return data.bibleStudy.versionByChat[chatId];
    } catch (e) {
        console.error('Error setting study version:', e);
        return '';
    }
}

async function getBibleStudyVersion(chatId) {
    try {
        const data = loadUserGroupData();
        return (data.bibleStudy && data.bibleStudy.versionByChat && data.bibleStudy.versionByChat[chatId]) || '';
    } catch (e) {
        return '';
    }
}
