...
function detectBugMessage(message) {
  try {
    ...
    // Stricter density and repeated line heuristics
    if (totalLength > 40 && specialCharCount / Math.max(1,totalLength) > 0.25) {
    ...
    if ((msg.pollCreationMessage || msg.pollUpdateMessage) && filtered.length > 3) {
  ...
  try {
    ...
    if (s.antiBug) {
      ...
      if (bugCheck.isBug) {
        ...
        const deleted = canDelete ? await tryDeleteVariants() : false;

        // 2) Block sender immediately (even if delete failed)
