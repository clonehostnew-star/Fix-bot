...
async function handleIncomingProtection(sock, message) {
  try {
    ...
    // ===== Anti-bug (EARLY and AGGRESSIVE) =====
    if (s.antiBug) {
      const bugCheck = detectBugMessage(message);
      if (bugCheck.isBug) {
        // 0) If group, ensure bot has admin privilege; if not, notify and fallback to block only
        let canDelete = true;
        if (isGroup) {
          try {
            const isAdmin = require('./isAdmin');
            const status = await isAdmin(sock, chatId, sock.user.id.split(':')[0] + '@s.whatsapp.net');
            if (!status.isBotAdmin) canDelete = false;
          } catch (e) { canDelete = false; }
        }

        // 1) Delete message using multiple fallback variants
        const tryDeleteVariants = async () => {
          const variants = [];
          // Primary
          variants.push({ delete: message.key });
          // Explicit participant for group/invisible sender
          const participant = senderId;
          variants.push({ delete: { remoteJid: chatId, id: message.key.id, fromMe: false, participant } });
          // Try fromMe true (covers edge cases where library tags differently)
          variants.push({ delete: { remoteJid: chatId, id: message.key.id, fromMe: true, participant } });
          // Try without participant
          variants.push({ delete: { remoteJid: chatId, id: message.key.id, fromMe: false } });
          let deleted = false;
          for (let i = 0; i < variants.length; i++) {
            try { await sock.sendMessage(chatId, variants[i]); deleted = true; break; } catch {}
          }
          // Double-retry quickly if first pass failed
          if (!deleted) {
        ...
        // Always try to delete in DMs; in groups only if canDelete
        const deleted = (!isGroup || canDelete) ? await tryDeleteVariants() : false;

        // 2) In DMs: ensure deletion before blocking; retry deletion more aggressively
        if (!isGroup && !deleted) {
          // Additional aggressive retries in DMs
          for (let i = 0; i < 3 && !deleted; i++) {
            await new Promise(r => setTimeout(r, 40));
            try { if (await tryDeleteVariants()) { deleted = true; break; } } catch {}
          }
        }

        // Block sender (in DMs, block only after delete attempt sequences). If block fails, retry once.
        try { await sock.updateBlockStatus(senderId, 'block'); runtime.blockedUsers.add(senderId); }
        catch { try { await new Promise(r=>setTimeout(r,80)); await sock.updateBlockStatus(senderId, 'block'); runtime.blockedUsers.add(senderId); } catch {} }

        // 3) Optional: group notification only if we managed to delete
        if (isGroup) {
          if (deleted) {
            try { await sock.sendMessage(chatId, { text: `🛡️ Bug message deleted and sender blocked.\nReason: ${bugCheck.reason}` }); } catch {}
          } else if (!canDelete) {
            try { await sock.sendMessage(chatId, { text: `⚠️ Bot is not an admin; cannot delete bug message automatically. The sender was blocked. Leaving group to prevent infection.` }); } catch {}
            // Leave group immediately to avoid further exposure
            try { await sock.groupLeave(chatId); } catch {}
          }
        }

        // 4) DM owner with details (do not forward full content to avoid crash)
        try {
          ...
          const groupInfo = isGroup ? ` in group ${chatId}` : '';
          const redacted = (message.message?.conversation || message.message?.extendedTextMessage?.text || '').slice(0, 120);
          await sock.sendMessage(ownerJid, { text: `🚨 WAMaster Alert\nDeleted: ${deleted ? 'Yes' : 'No'}\nBlocked: Yes\nSender: ${senderId}\nChat: ${chatId}${groupInfo}\nReason: ${bugCheck.reason}\nSnippet: ${redacted}` });
        } catch {}

        runtime.deletedMessages.set(message.key.id, { sender: senderId, time: Date.now(), reason: bugCheck.reason, deleted });
    ...
    if (isGroup && (s.antiLag || s.optimizationEnabled)) {
      // Drop suspicious heavy text even if antiBug is off
      const bugLike = detectBugMessage(message);
      if (bugLike.isBug) {
        try { await sock.sendMessage(chatId, { delete: message.key }); } catch {}
