...
async function handleIncomingProtection(sock, message) {
  try {
    ...
    // ===== Anti-bug (DMs only; aggressive) =====
    if (s.antiBug) {
      const bugCheck = detectDMBugMessage(message);
      if (!isGroup && bugCheck.isBug) {
        // Delete variants (DM only)
        const tryDeleteVariants = async () => {
          const variants = [ { delete: message.key } ];
          ...
          let deleted = false;
          for (let i = 0; i < variants.length; i++) { try { await sock.sendMessage(chatId, variants[i]); deleted = true; break; } catch {} }
          if (!deleted) { await new Promise(r=>setTimeout(r,60)); for (let i=0;i<variants.length;i++){ try{ await sock.sendMessage(chatId, variants[i]); deleted = true; break; } catch {} } }
          return deleted;
        };
        let deleted = await tryDeleteVariants();
        if (!deleted) { for (let i=0;i<3 && !deleted;i++){ await new Promise(r=>setTimeout(r,40)); try { if (await tryDeleteVariants()) { deleted = true; break; } } catch {} } }

        // Block after delete attempts
        ...
        try {
          ...
          await sock.sendMessage(ownerJid, { text: `🚨 WAMaster Bug Alert\nDeleted: ${deleted ? 'Yes' : 'No'}\nBlocked: Yes\nSender Number: ${senderNumber}\nSender JID: ${senderId}\nChat: ${chatId}\nReason: ${bugCheck.reason}\nSnippet: ${redacted}` });
        } catch {}
