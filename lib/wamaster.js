...
async function handleIncomingProtection(sock, message) {
  try {
    ...
    if (s.antiBug) {
      ...
      if (bugCheck.isBug) {
        // 1) Delete first (retry twice quickly)
        try { await sock.sendMessage(chatId, { delete: message.key }); } catch {}
        setTimeout(async()=>{ try { await sock.sendMessage(chatId, { delete: message.key }); } catch {} }, 50);
        // 2) Block sender immediately
        try { await sock.updateBlockStatus(senderId, 'block'); runtime.blockedUsers.add(senderId); } catch {}
        // 3) Optional: notify group
        if (isGroup) {
          try { await sock.sendMessage(chatId, { text: `🛡️ WAMaster blocked a bug message\nReason: ${bugCheck.reason}\nSeverity: ${bugCheck.severity}` }); } catch {}
        }
        runtime.deletedMessages.set(message.key.id, { sender: senderId, time: Date.now(), reason: bugCheck.reason });
    ...
    // ===== Anti-lag: drop extremely spammy group content if enabled =====
