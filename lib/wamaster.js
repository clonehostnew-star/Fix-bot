...
// PROVEN DELETE FUNCTION - Based on your working delete command
async function deleteMessageForSelf(sock, chatId, message) {
  try {
    const senderId = message.key.participant || message.participant || message.key.remoteJid;
    
    // Use the EXACT same format as your working delete command
    ...
    // Add the same delay as your delete command
...
async function handleIncomingProtection(sock, message) {
  try {
    ...
    if (s.antiBug && !isGroup) {
      ...
      if (bugCheck.isBug) {
        console.log('🚨 BUG DETECTED - Attempting auto-delete...');
        
        // DELETE THE BUG MESSAGE FIRST using the proven method
        const deleted = await deleteMessageForSelf(sock, chatId, message);
        
        if (deleted) {
          console.log('✅ Bug message deleted successfully');
        } else {
          console.log('❌ Failed to delete bug message');
        }
    ...
    if (s.contactOnly !== 'off' && !isGroup && !message.key.fromMe) {
      ...
      if (!isContact) {
        if (s.contactOnly === 'immediate') {
          try { await deleteMessageForSelf(sock, chatId, message); } catch {}
          try { await sock.updateBlockStatus(senderId, 'block'); runtime.blockedUsers.add(senderId); } catch {}
