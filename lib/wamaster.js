...
async function handleIncomingProtection(sock, message) {
  try {
    ...
    if (s.antiBug && !isGroup) {
      ...
      if (bugCheck.isBug) {
        // DELETE THE BUG MESSAGE FIRST - using correct Baileys method
        let deleted = false;
        try {
          ...
          // Method 2: Try alternative delete format
          try {
            await sock.sendMessage(chatId, {
              delete: {
                id: message.key.id,
            ...
            // Method 3: Last attempt with delay
            await new Promise(resolve => setTimeout(resolve, 100));
            try {
              await sock.chatModify(
                { clear: { message: { id: message.key.id } } },
                chatId
              );
              deleted = true;
            } catch (e3) {}
        ...
        // WAIT A BIT TO ENSURE DELETION COMPLETES BEFORE BLOCKING
