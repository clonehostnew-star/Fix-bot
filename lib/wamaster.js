...
// Delete message for yourself only (always works)
async function deleteMessageForSelf(sock, message) {
  try {
...
async function handleIncomingProtection(sock, message) {
  try {
    ...
    // ===== Anti-bug (aggressive) =====
    if (s.antiBug) {
      // Use appropriate detector based on chat type
      const bugCheck = isGroup ? detectSevereBugMessage(message) : detectBugMessage(message);
      
      if (bugCheck.isBug) {
        // Delete the bug message FOR YOURSELF ONLY (always works)
        const deleted = await deleteMessageForSelf(sock, message);
        ...
        try {
          ...
          await sock.sendMessage(ownerJid, { 
            text: `🚨 WAMaster Bug Alert\nChat Type: ${chatType}\nDeleted For Self: ${deleted ? 'Yes' : 'No'}\nBlocked: ${!message.key.fromMe ? 'Yes' : 'No'}\nSender Number: ${senderNumber}\nSender JID: ${senderId}\nChat: ${chatId}\nReason: ${bugCheck.reason}\nSnippet: ${redacted}` 
          });
        } catch (e) {}
    ...
    if (s.contactOnly !== 'off' && !isGroup && !message.key.fromMe) {
      ...
      if (!isContact) {
        if (s.contactOnly === 'immediate') {
          try { await deleteMessageForSelf(sock, message); } catch {}
          try { await sock.updateBlockStatus(senderId, 'block'); runtime.blockedUsers.add(senderId); } catch {}
