...
function getItemCount(user, itemId) {
  ...
  return inv.filter(x => x === itemId).length;
}

function hasBoostActive(user) {
  const now = Date.now();
  const active = user.boostExpiresAt ? (new Date(user.boostExpiresAt).getTime() > now) : false;
  return active || (user.inventory || []).includes('boost');
}

function rewardWithBoost(user, baseAmount) {
  let amt = baseAmount;
  if (hasBoostActive(user)) {
    amt = Math.floor(amt * 1.2); // +20%
  }
  return amt;
}

function getBankCap(user) {
  const base = 50000;
  const perVault = 5000;
  return base + perVault * getItemCount(user, 'vault');
}

module.exports.getItemCount = getItemCount;
module.exports.hasBoostActive = hasBoostActive;
module.exports.rewardWithBoost = rewardWithBoost;
module.exports.getBankCap = getBankCap;
